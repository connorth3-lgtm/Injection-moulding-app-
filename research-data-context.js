/* MouldMaster research ↔ semantic process-data context bridge — 2026.09.02.5 */
(function(){
'use strict';
...
const unitText=v=>clean(v)?` ${clean(v)}`:'';
...
function ensureRunStyle(){
  if(document.getElementById('mm-run-insights-style'))return;
  const s=document.createElement('style');s.id='mm-run-insights-style';s.textContent=`
.mm-run-insights{margin:12px 0;border:1px solid #3c6388;border-radius:14px;background:linear-gradient(145deg,#10243a,#0b192a);padding:16px}.mm-run-insights h3{font-size:22px;margin:4px 0 6px}.mm-ri-lead{color:#c5d7e8;line-height:1.55;margin:0 0 12px}.mm-ri-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:9px}.mm-ri-card{border:1px solid #2e4b68;border-radius:10px;background:#0d1d30;padding:11px}.mm-ri-card>small{display:block;color:#89a3bd;text-transform:uppercase;letter-spacing:.07em;font-size:9px}.mm-ri-card>b{display:block;margin:5px 0;font-size:14px}.mm-ri-card p{margin:4px 0;color:#bdd0e2;font-size:11px;line-height:1.45}.mm-ri-signal{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;padding:7px 8px;margin-top:5px;border:1px solid #2c465f;border-radius:8px;background:#0a1828;font-size:11px}.mm-ri-signal b{display:block}.mm-ri-signal small{display:block;color:#8fa8c0;margin-top:2px}.mm-ri-signal span:last-child{font-weight:800;text-align:right}.mm-ri-high span:last-child{color:#ff9da8}.mm-ri-review span:last-child{color:#ffd166}.mm-ri-stable span:last-child{color:#7ce6a3}.mm-ri-readiness{display:flex;gap:6px;flex-wrap:wrap;margin-top:10px}.mm-ri-chip{border:1px solid #405e7d;border-radius:999px;padding:4px 7px;font-size:9px}.mm-ri-chip.ok{border-color:#39765e;color:#8ee7be}.mm-ri-chip.missing{border-color:#795d36;color:#ffd18a}.mm-ri-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.mm-ri-detail{margin-top:10px;border-top:1px solid #2b4864;padding-top:10px}.mm-ri-detail summary{cursor:pointer;font-weight:800}.mm-ri-engineer{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px}.mm-ri-note{padding:9px;border-left:3px solid #557ca5;background:#0b1929;font-size:11px;line-height:1.5;color:#bad0e4}.mm-ri-practice{margin-top:14px;padding:14px;border:1px solid #42607d;border-radius:12px;background:#0b1a2b}.mm-ri-practice h4{margin:4px 0 6px;font-size:16px}.mm-ri-practice>p{margin:0;color:#b9cce0;font-size:12px;line-height:1.5}.mm-ri-practice-meta{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}.mm-ri-choice-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:10px}.mm-ri-choice{min-height:68px;padding:10px;border:1px solid #35516e;border-radius:9px;background:#102238;color:#e8f2fc;text-align:left;font-size:11px;line-height:1.45}.mm-ri-choice:hover,.mm-ri-choice:focus-visible{border-color:#69a8ff;background:#142a43}.mm-ri-choice.correct{border-color:#3d8b6c;background:#102a24}.mm-ri-choice.wrong{border-color:#8a4d58;background:#2d171d}.mm-ri-choice:disabled{cursor:default;opacity:1}.mm-ri-feedback{display:none;margin-top:10px;padding:11px;border-left:3px solid #5a83aa;background:#0e2033;color:#c4d7e8;font-size:11px;line-height:1.55}.mm-ri-feedback[data-open="1"]{display:block}.mm-ri-feedback b{color:#edf6ff}.mm-ri-ruleout{margin-top:7px;padding-top:7px;border-top:1px solid #29445f}@media(max-width:850px){.mm-ri-grid,.mm-ri-engineer{grid-template-columns:1fr}}@media(max-width:620px){.mm-ri-choice-grid{grid-template-columns:1fr}}@media(max-width:520px){.mm-ri-signal{grid-template-columns:1fr}.mm-ri-signal span:last-child{text-align:left}}`;
...
function changeFact(x){const baseline=Number(x?.baselineMean),current=Number(x?.currentMean),delta=current-baseline,epsilon=Math.max(Math.abs(baseline),Math.abs(current),1)*1e-12;return {baseline,current,delta,absDelta:Math.abs(delta),direction:Math.abs(delta)<=epsilon?'held':delta>0?'rose':'fell',arrow:Math.abs(delta)<=epsilon?'→':delta>0?'↑':'↓',unit:clean(x?.unit),sigma:Number(x?.normalizedShift)}}
function changeSentence(x){const f=changeFact(x),name=clean(x?.meaning||x?.channel)||'Signal';return `${name} ${f.direction}${Number.isFinite(f.absDelta)?` ${fmt(f.absDelta)}${unitText(f.unit)}`:''}${Number.isFinite(f.sigma)?` (${f.arrow} ${fmt(f.sigma)}σ)`:''}`}
function changeMarkup(changes){return changes.slice(0,4).map(x=>{const f=changeFact(x);return `<div class="mm-ri-signal mm-ri-${esc(x.level||'review')}"><div><b>${esc(x.meaning||x.channel)}</b><small>Baseline ${fmt(f.baseline)} → current ${fmt(f.current)}${esc(unitText(f.unit))}</small></div><span>${esc(f.arrow)} ${fmt(f.absDelta)}${esc(unitText(f.unit))} · ${fmt(f.sigma)}σ</span></div>`}).join('')}
...
async function saveRunCase(dataset,drift,mechanism,plan){
  ...
  const changes=driftRows(drift).slice(0,4).map(x=>{const f=changeFact(x);return `${x.meaning||x.channel}: ${fmt(f.baseline)} to ${fmt(f.current)}${unitText(f.unit)} (${f.arrow} ${fmt(f.absDelta)}${unitText(f.unit)}, ${fmt(f.sigma)} sigma)`}).join('; ');
...
function practiceMarkup(practice){
  if(!practice)return'';
  return `<section class="mm-ri-practice" data-mm-ri-practice="${esc(practice.id)}"><span class="eyebrow">Reason it through</span><h4>Choose the strongest next evidence</h4><p>${esc(practice.prompt)}</p><div class="mm-ri-practice-meta"><span class="mm-ri-chip ok">Formative only</span><span class="mm-ri-chip">Does not affect formal assessment</span></div><div class="mm-ri-choice-grid">${practice.options.map((x,i)=>`<button type="button" class="mm-ri-choice" data-mm-ri-choice="${i}">${esc(x.text)}</button>`).join('')}</div><div class="mm-ri-feedback" data-mm-ri-feedback aria-live="polite"></div></section>`
}
function wireRunPractice(section,practice){
  if(!practice)return;
  const host=section.querySelector('[data-mm-ri-practice]'),feedback=host?.querySelector('[data-mm-ri-feedback]');if(!host||!feedback)return;
  const startedAt=Date.now();
  host.querySelectorAll('[data-mm-ri-choice]').forEach(button=>button.addEventListener('click',()=>{
    if(host.dataset.mmAnswered==='1')return;host.dataset.mmAnswered='1';
    const chosenIndex=Number(button.dataset.mmRiChoice),chosen=practice.options[chosenIndex],correct=chosenIndex===practice.correctIndex;
    const buttons=[...host.querySelectorAll('[data-mm-ri-choice]')];buttons.forEach((b,i)=>{b.disabled=true;if(i===practice.correctIndex)b.classList.add('correct');else if(i===chosenIndex)b.classList.add('wrong')});
    feedback.dataset.open='1';feedback.innerHTML=`<b>${correct?'Good evidence reasoning.':'A stronger check is available.'}</b> ${esc(chosen?.feedback||practice.rationale)}<div class="mm-ri-ruleout"><b>${esc(practice.weakeningQuestion)}</b><br>${esc(practice.weakeningAnswer)}</div><div class="mm-ri-ruleout"><b>What recovery should look like</b><br>${esc(practice.recoveryAnswer)}</div><p>${esc(practice.boundary)}</p>`;
    const seconds=Math.max(0,Math.round((Date.now()-startedAt)/1000)),analytics=window.MM_LEARNING_ANALYTICS;
    analytics?.record?.('practice_start',{module:'process-data',id:practice.id,attempt:1});
    if(!correct)analytics?.record?.('practice_miss',{module:'process-data',id:practice.id,step:2,correct:false});
    analytics?.record?.('practice_complete',{module:'process-data',id:practice.id,score:correct?100:0,durationSec:seconds,correct});
    window.MM_RESEARCH_UTILISATION?.record?.('run_learning_answered',{mechanismId:practice.mechanismId,surface:'run-insights',applicability:correct?'reasoned-correct':'reasoned-review'});
  }))
}
async function renderRunInsights(root,dataset,baseContext){
  ...
  const baselineSelect=root.querySelector('[data-pi-baseline]'),baselineId=clean(baselineSelect?.value)||[...baselineSelect?.options||[]].map(x=>x.value).find(Boolean)||'';
  ...
  wireRunPractice(section,practice);
...
async function refreshRunInsights(root,dataset,context){root.querySelector('[data-mm-run-insights]')?.remove();await renderRunInsights(root,dataset,context)}
async function wireProcessIntelligence(){
  ...
  const baselineSelect=root.querySelector('[data-pi-baseline]');if(baselineSelect&&baselineSelect.dataset.mmRunInsightsWired!=='1'){baselineSelect.dataset.mmRunInsightsWired='1';baselineSelect.addEventListener('change',()=>refreshRunInsights(root,match.dataset,match.context).catch(()=>{}))}
...
function workspaceCaseFromDom(root){const out={};root.querySelectorAll('[data-mw-field]').forEach(el=>out[el.dataset.mwField]=el.value);return out}
...
window.MM_RESEARCH_DATA_CONTEXT={version:VERSION,datasetContext,forDataset,intakeContext,renderRunInsights,scope:'Connects promoted research mechanisms to semantic process-data metadata and troubleshooting cases. Site-process views are technician-first: measured baseline-to-current change with direction and engineering units, likely explanation, next check, rule-out evidence and formative evidence-selection practice appear before detailed research. The decision brief follows the user-selected local baseline. Run-linked practice stays outside the formal assessment bank. No raw rows are exposed and no production authority is created.'};
})();
