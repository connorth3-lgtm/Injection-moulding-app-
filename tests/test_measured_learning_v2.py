...
class MeasuredLearningV2Tests(unittest.TestCase):
    def binding(self):
        return {
            ...
            "extraction": {
                "description": "first four published rows used only as deterministic QA fixture",
                "sourceArtifact": "Real_World_Test/Case_Study_Raw_Data.csv",
                "sourceMember": None,
                "sourceOrderingPreserved": True,
                "window": {"kind":"range","axis":"row-index","scope":"Case_Study_Raw_Data.csv","start":0,"endExclusive":4,"unit":"row"},
            },
            "signals": [
                {
                    "id":"pressure","sourceChannel":"P","label":"Cavity pressure","semantic":"cavity-pressure","unit":"bar",
                    "representation":{"originalPointCount":4,"reductionMethod":"identity-v1","xSemantic":"time","xUnit":"s","x":[0.015883300000041345,0.11717850000002272,0.30457799999999224,0.3722109000000273],"y":[0.0,0.0,0.0,0.0]},
                },
                {
                    "id":"force","sourceChannel":"F","label":"Extraction force","semantic":"extraction-force","unit":"N",
                    "representation":{"originalPointCount":4,"reductionMethod":"identity-v1","xSemantic":"time","xUnit":"s","x":[0.015883300000041345,0.11717850000002272,0.30457799999999224,0.3722109000000273],"y":[0.0,109.45,109.45,27.36]},
                },
            ],
    ...
    def test_valid_fixture_builds_and_revalidates(self):
        ...
        self.assertEqual(case["schemaVersion"], 3)
        self.assertAlmostEqual(case["features"][0]["value"], 109.45)
    ...
    def test_wrong_artifact_hash_fails(self):
        ...
        binding["sourceFingerprint"] = "sha256:" + "0" * 64
        with self.assertRaises(SystemExit):
            builder.build(self.candidate(), binding)

    def test_wrong_channel_unit_fails(self):
        binding = self.binding()
        binding["signals"][0]["unit"] = "MPa"
        with self.assertRaises(SystemExit):
            builder.build(self.candidate(), binding)
    ...
    def test_tampered_feature_is_rejected_even_with_new_case_hash(self):
        case = builder.build(self.candidate(), self.binding())
        case["features"][0]["value"] += 1.0
        case["caseFingerprint"] = canonical_sha({k:v for k,v in case.items() if k != "caseFingerprint"})
    ...
    def test_raw_window_identity_is_separate_from_representation(self):
        ...
        changed = copy.deepcopy(binding["signals"])
        changed[1]["representation"]["y"][1] += 10
        raw2 = raw_window_fingerprint(binding["sourceFingerprint"], extraction["sourceArtifact"], None, extraction)
        self.assertEqual(raw1, raw2)
        self.assertNotEqual(representation_fingerprint(raw1, binding["signals"]), representation_fingerprint(raw2, changed))

    def test_substantial_overlap_is_detected(self):
        a = {"kind":"range","axis":"row-index","scope":"x","start":0,"endExclusive":100,"unit":"row"}
        b = {"kind":"range","axis":"row-index","scope":"x","start":10,"endExclusive":100,"unit":"row"}
