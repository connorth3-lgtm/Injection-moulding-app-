...
REGISTRY = json.loads((ROOT / 'data/book-evidence-registry-v1.json').read_text(encoding='utf-8'))
...
expected = {'polymer-structure','rheology','moisture-drying','thermal-history','material-families','fillers-additives','plasticising-unit','shot-utilisation','velocity-pressure'}
chapters = BATCH['chapters']
assert {c['id'] for c in chapters} == expected
manifest_chapters = {c['id']: c for p in MANIFEST['parts'] for c in p.get('chapters', [])}
source_ids = {s['id'] for s in MANIFEST['sourceSeeds']} | {s['id'] for s in REGISTRY.get('sourceSeeds', [])}
for chapter in chapters:
    ...
    assert all(sid in source_ids for sid in chapter['sourceIds']), f'unknown source in {cid}: {chapter["sourceIds"]}'
