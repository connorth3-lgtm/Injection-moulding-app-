...
data=json.loads(TARGET_FILE.read_text(encoding='utf-8')); assert data['targets']['research_evidence_domains']>=600; assert data['execution_state']['cumulative_passes']==600; assert data['execution_state']['wave6_primary_seeded']==92; assert data['execution_state']['wave6_explicit_gaps']==8
...
seed=SEED_FILE.read_text(encoding='utf-8'); assert len(re.findall(r'^\| .*?\| https?://',seed,flags=re.M))>=6
mins=[78,59,93,69,95,92]; titles=[]; counts={}
for i,p in enumerate(WAVE_FILES,1):
 o=json.loads(p.read_text(encoding='utf-8')); start=1 if i==1 else (i-1)*100+1; end=i*100; assert o['pass_count']==100
...
doc=WAVE6_DOC.read_text(encoding='utf-8')
