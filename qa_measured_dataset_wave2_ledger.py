...
need(summary.get("wave2FullyProfiledAccepted") == 5, "Wave-2 accepted family delta drifted")
need(summary.get("effectiveFullyProfiledMeasuredDatasetFamilies") == 12, "Wave-2 effective family count drifted")
need(summary.get("effectiveAcceptedMeasuredTimeSeriesSamples") == 66521519, "Wave-2 must not inflate process waveform count")
need(summary.get("wave2RecordLevelMeasuredOutcomeValues") == 1110, "Wave-2 direct record-level outcome count drifted")
...
need(TARGETS["fully_profiled_measured_datasets"]["currentAccepted"] == 12, "target family count is not reconciled")
need(TARGETS["fully_profiled_measured_datasets"]["currentDiscovered"] == 25, "target discovered count is not reconciled")
need(TARGETS["measured_time_series_samples"]["currentAccepted"] == 66521519, "target process waveform count drifted")
need(INV["summary"]["datasets"] == 25 and INV["summary"]["automatedIngestionAllowed"] == 14, "Wave-2 inventory reconciliation drifted")
by_id = {x["datasetId"]: x for x in INV["datasets"]}
for did in ["mendeley-fhj5p7ww9v-v1","mendeley-6k8fpbrd9s-v1","mendeley-4h98rz9f92-v3","pmc4753395-hdpe-cenosphere-v1","mendeley-yxz2w7ctnh-v1"]:
...
need(by_id["mendeley-yxz2w7ctnh-v1"]["count"].get("directRecordLevelInjectionMeasuredValues") == 489, "yxz2 direct record-level injection mechanical-test count drifted")
need(by_id["mendeley-yxz2w7ctnh-v1"]["automatedIngestionAllowed"] is True, "yxz2 CC BY source should be executable")
print("Wave-2 family ledger QA passed (7 -> 12 families; 25 inventoried sources; 14 executable; process waveform total unchanged at 66,521,519)")
