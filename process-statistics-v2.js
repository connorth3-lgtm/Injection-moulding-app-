/* MouldMaster advanced process-statistics evidence layer — 2026.09.02.1 */
(function(root,factory){
'use strict';
const api=factory();
if(typeof module==='object'&&module.exports)module.exports=api;
if(root){
  if(root.MM_PROCESS_STATISTICS_V2)return;
  root.MM_PROCESS_STATISTICS_V2=Object.freeze({...api,version:'2026.09.02.1'});
  api.install(root);
}
...
function summary(values,totalRows=null){const a=finite(values),m=mean(a),s=sd(a),rows=Number.isFinite(Number(totalRows))?Number(totalRows):(values||[]).length;return{n:a.length,rows,missing:Math.max(0,rows-a.length),missingRate:rows?Math.max(0,rows-a.length)/rows:null,mean:m,sd:s,min:a.length?Math.min(...a):null,max:a.length?Math.max(...a):null}}
function lag1Autocorrelation(values){
  const a=finite(values);if(a.length<4)return{n:a.length,r:null,strength:'insufficient'};
...
function sameSide(v,c,threshold,sigma){return v>c+threshold*sigma?1:v<c-threshold*sigma?-1:0}
function spcRunRules(values,centre=null,sigma=null){
  ...
  for(let i=0;i<=a.length-3;i++){
    const w=a.slice(i,i+3),hi=w.filter(v=>sameSide(v,c,2,s)===1).length,lo=w.filter(v=>sameSide(v,c,2,s)===-1).length;
  ...
  for(let i=0;i<=a.length-5;i++){
    const w=a.slice(i,i+5),hi=w.filter(v=>sameSide(v,c,1,s)===1).length,lo=w.filter(v=>sameSide(v,c,1,s)===-1).length;
  ...
  for(let i=0;i<=a.length-14;i++){
    const w=a.slice(i,i+14),alternating=w.slice(1).every((v,j)=>(v-w[j])*(w[j]-(w[j-1]??w[j]))<0);
    if(alternating)push('14-alternating',i,i+13,'alternating','Fourteen consecutive points alternate direction; investigate cyclic/over-control patterns before interpreting independence.');
  }
...
function meanDifference(before,after){
  const a=finite(before),b=finite(after),ma=mean(a),mb=mean(b),sa=sd(a),sb=sd(b);if(!a.length||!b.length)return{nBefore:a.length,nAfter:b.length,meanBefore:ma,meanAfter:mb,difference:null,ci95:null,effectSize:null};
...
function stratify(rows,channel){
  const out={};for(const [dimension,candidates] of Object.entries(DIMENSIONS)){
    const key=dimensionKey(rows,candidates);if(!key)continue;const groups=new Map();let unlabelled=0;
...
function cavityVariance(rows,channel){
  const key=dimensionKey(rows,['cavity','cavity_id']);if(!key)return{available:false,reason:'No retained cavity identifier'};const groups=new Map();
  ...
  const usable=[...groups].filter(([,a])=>a.length>=2);if(usable.length<2)return{available:false,reason:'At least two cavities with two numeric observations each are required'};
  ...
  const withinVariance=N>k?withinSS/(N-k):null,betweenVariance=k>1?betweenSS/(k-1):null,varianceRatio=Number.isFinite(withinVariance)&&withinVariance>0&&Number.isFinite(betweenVariance)?betweenVariance/withinVariance:null;
...
function channelDiagnostics(rows,channel,reference={}){
  const values=(rows||[]).map(r=>r?.[channel]),s=summary(values,rows?.length||0),acf=lag1Autocorrelation(values),spc=spcRunRules(values,reference.centre,reference.sigma),strata=stratify(rows,channel),cavity=cavityVariance(rows,channel);
...
function windowDiagnostics(rows,channel,splitIndex,windowSize=20){
  const i=Math.max(1,Math.min((rows||[]).length-1,Number(splitIndex)||Math.floor((rows||[]).length/2))),n=Math.max(3,Math.min(500,Number(windowSize)||20)),beforeRows=(rows||[]).slice(Math.max(0,i-n),i),afterRows=(rows||[]).slice(i,Math.min((rows||[]).length,i+n)),before=beforeRows.map(r=>r?.[channel]),after=afterRows.map(r=>r?.[channel]),bs=summary(before,beforeRows.length);
...
function install(root){
  ...
  function attach(){
    ...
    intel.compareToBaseline=async function(datasetId,baselineId){
      ...
      result.advancedStatistics={version:'2026.09.02.1',boundary:BOUNDARY};latest.drift=result;schedule();return result
    };
    intel.compareWindows=function(rows,semantics,splitIndex,windowSize=20){const result=baseWindows(rows,semantics,splitIndex,windowSize);for(const change of result?.changes||[])change.advancedStatistics=windowDiagnostics(rows,change.channel,splitIndex,windowSize);result.advancedStatistics={version:'2026.09.02.1',boundary:BOUNDARY};latest.windows=result;schedule();return result};
    intel.__mmStatisticsV2=true;root.MM_PROCESS_STATISTICS_V2_RUNTIME=Object.freeze({version:'2026.09.02.1',installed:true,boundary:BOUNDARY});return true
  }
