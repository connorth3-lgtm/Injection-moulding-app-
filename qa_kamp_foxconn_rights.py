...
need(review.get("version") == "2026.08.30.1", "KAMP/Foxconn rights-review version drifted")
need(review.get("reviewed") == "2026-08-30", "KAMP/Foxconn review date drifted")
...
need(kcatalog.get("catalogRows") == 50 and kcatalog.get("catalogFormat") == "CSV", "KAMP official catalog dimensions drifted")
need(kcatalog.get("catalogFileDownloadWithoutLogin") is True, "KAMP government catalog should remain no-login downloadable")
need(kcatalog.get("catalogPermissionScope") == "이용허락범위 제한 없음", "KAMP government catalog permission scope drifted")
need(kcatalog.get("rowSchemaIncludesUsageCondition") is True, "KAMP per-row usage-condition schema evidence missing")
need("사용조건" in (kcatalog.get("rowSchemaFields") or []), "KAMP official row schema lost 사용조건")
need("데이터셋 다운 링크" in (kcatalog.get("rowSchemaFields") or []), "KAMP official row schema lost download link")
...
need(kcatalog.get("officialRawCsvEndpointExposedByPublicHtml") is False, "KAMP raw-CSV endpoint boundary drifted")
need("콘텐츠 변경허용" in str(kcatalog.get("exactRowUsageConditionStatus", "")), "KAMP secondary usage-condition evidence missing")
analysis_intent = kamp.get("publicAnalysisIntentEvidence") or {}
need("KAMP" in str(analysis_intent.get("historicalMinistryEvidence", "")), "KAMP Ministry analysis-intent evidence missing")
need("50" in str(analysis_intent.get("current2026CompetitionEvidence", "")), "KAMP current 50-dataset competition evidence missing")
need("not accepted" in str(analysis_intent.get("boundary", "")), "KAMP analysis-intent evidence must not substitute for dataset-specific rights")
...
fresh = fox.get("freshCorroboration") or {}
need(fresh.get("checkedOn") == "2026-08-30", "Foxconn corroboration date drifted")
need("does not establish reuse permission" in str(fresh.get("rightsImpact", "")), "Foxconn fresh corroboration must remain non-licensing")
...
need("exact first-party DATASET_SEQ=4" in boundary, "KAMP exact remaining proof must stay explicit")

print("KAMP/Foxconn source-rights QA passed (KAMP narrowed to exact first-party row capture; Foxconn remains without enduring reuse licence; both fail-closed)")
