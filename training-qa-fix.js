...
const ANALYTICS_PREFIXES=['mm_assessment_analytics_v1','mm_assessment_exposure_timing_v1','mm_assessment_opening_history_v1'];
...
/* Best-effort atomic import: validate and serialise first, then roll back every core/training store if a write fails. Assessment-derived stores are not part of the backup and are cleared only after a successful import. */
window.importData=function(file){
 ...
 r.onload=()=>{
  ...
  try{
   ...
   window.toast?.('Progress imported. Certificates must be re-earned; local assessment analytics and rotation history were reset.');
  }catch(e){
...
/* Confirmed factory reset clears every MouldMaster-owned training and assessment-derived store. */
