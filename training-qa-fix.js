/* MouldMaster training data/assessment bridge — 2026.09.05.1 */
(function(){
'use strict';
...
function clearAssessmentAnalyticsStores(){
 try{
  for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i);if(k&&ASSESSMENT_ANALYTICS_PREFIXES.some(p=>k===p||k.startsWith(p+'::')))localStorage.removeItem(k)}
 }catch(e){console.warn('[MouldMaster] assessment analytics cleanup:',e)}
}
function clearLearningAnalyticsStores(){
 try{
  for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i);if(k?.startsWith(LEARNING_ANALYTICS_PREFIX))localStorage.removeItem(k)}
 }catch(e){console.warn('[MouldMaster] learning analytics cleanup:',e)}
}
function clearAllAnalyticsStores(){clearAssessmentAnalyticsStores();clearLearningAnalyticsStores()}
...
/* Best-effort atomic import: validate and serialise first, then roll back every core/training store if a write fails. Analytics are not part of the backup and are cleared only after a successful import so stale device/profile evidence cannot be attributed to imported learners. */
window.importData=function(file){
 ...
 r.onload=()=>{
  ...
  try{
   ...
   db=proposed;user=db.users[db.activeUser];committed=true;cancelActiveExam();clearAllAnalyticsStores();
   ...
   window.toast?.('Progress imported. Certificates must be re-earned; local assessment and Learning Insights analytics were reset.');
  }catch(e){
...
/* Confirmed factory reset clears every MouldMaster-owned training and analytics store. */
const baseReset=window.resetData;if(typeof baseReset==='function')window.resetData=function(){const beforeDb=typeof db==='undefined'?null:db,r=baseReset.apply(this,arguments);setTimeout(()=>{if(typeof db!=='undefined'&&db!==beforeDb){localStorage.removeItem(REVIEW_KEY);localStorage.removeItem(LEGACY_REVIEW);localStorage.removeItem(SIGN_KEY);clearAllAnalyticsStores();cancelActiveExam()}},0);return r};
...
window.MM_TRAINING_DATA_BRIDGE={version:'2026.09.05.1',clearAssessmentAnalyticsStores,clearLearningAnalyticsStores,clearAllAnalyticsStores,cancelActiveExam};
})();