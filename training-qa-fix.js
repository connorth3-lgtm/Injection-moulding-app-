...
(function(){
  ...
  function safeParse(key,fallback){
    try{const v=JSON.parse(localStorage.getItem(key)||'');return v&&typeof v==='object'&&!Array.isArray(v)?v:fallback}catch(_){return fallback}
  }
  function cleanReview(v){
    ...
    for(const [id,x] of Object.entries(v.items).slice(0,1000)){
      ...
      out.items[String(id).slice(0,200)]={
        stage:Math.floor(num(x.stage,0,5,0)),due:num(x.due,0,4102444800000,Date.now()),wrong:Math.floor(num(x.wrong,0,100000,0)),right:Math.floor(num(x.right,0,100000,0)),last:num(x.last,0,4102444800000,0),confidence:['low','medium','high'].includes(x.confidence)?x.confidence:'medium',prompt:String(x.prompt||'').slice(0,1000)
      };
    }
    return out;
  }
  function cleanSign(v){
    ...
    out.supervisor=String(v.supervisor||'').slice(0,160);
    out.date=String(v.date||'').slice(0,20);
    out.notes=String(v.notes||'').slice(0,10000);
    return out;
  }
  ...
  window.exportData=function(){
    try{
      ...
      a.href=URL.createObjectURL(blob);a.download='mouldmaster-progress.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),0);
  ...
  if(typeof baseImport==='function'){
    window.importData=function(file){
      ...
      extrasReader.onload=()=>{try{
        ...
        if(x&&x.trainingExtras&&typeof x.trainingExtras==='object'){
  ...
  if(typeof baseReset==='function'){
    window.resetData=function(){
      ...
      setTimeout(()=>{
        const after=localStorage.getItem('mouldmasterProDB');
        if(before!==after){localStorage.removeItem(REVIEW_KEY);localStorage.removeItem(SIGN_KEY);window.activeExam=null;}
      },0);
      return result;
    };
  }
