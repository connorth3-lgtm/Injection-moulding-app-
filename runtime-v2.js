/* MouldMaster canonical runtime v2 — explicit core dispatch, module registry and scoped storage 2026-09-06 */
(function(){
'use strict';
...
function snapshot(){return {version:VERSION,learnerToken:storage.learnerToken(),modules:[...modules.values()].map(x=>({...x})),core:Object.fromEntries([...slots].map(([name,s])=>[name,{owner:s.owner,beforeHooks:s.before.size,afterHooks:s.after.size,bound:window[name]===s.dispatch}]))}}
window.MM_RUNTIME_V2=Object.freeze({version:VERSION,storage,setImplementation,restoreLegacy,rebind,before,after,registerModule,module:id=>modules.get(String(id))||null,snapshot,policy:'New MouldMaster features register through one explicit runtime dispatcher. A core implementation slot has one owner at a time; additive behavior uses named before/after hooks instead of wrapper chains. Compatibility layers may be rebound to the canonical dispatcher before final runtime composition.'});
