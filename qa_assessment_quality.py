...
for p in ['certification/QUALITY_AND_ASSESSMENT_MANUAL.md','sources/ASSESSMENT_SEMANTIC_REVIEW_CONTRACT.md','assessment-storage-scope.js','assessment-quality-suite.js','assessment-stable-review-bridge.js','assessment-analytics-ui.js','assessment-deep-dive.js','assessment-answer-cue-fix.js','training-upgrade.js','MouldMaster_Core_App.html','version.json','sources/QUESTION_BANK_CHANGELOG.md','sources/SOURCE_FRESHNESS.json','qa_source_freshness.py']:
...
p=subprocess.run(['node','--check',str(ROOT/'assessment-quality-suite.js')],capture_output=True,text=True,encoding='utf-8',errors='replace');need(p.returncode==0,f'assessment-quality-suite.js syntax error: {p.stderr}')
...
for marker in ["q.mmId=q.stableId","stableIdsPrimary:true","legacyRecordsMigratedBy:'assessment-quality-suite.js'"]: need(marker in bridge,f'stable-review bridge marker missing: {marker}')
p=subprocess.run(['node','--check',str(ROOT/'assessment-stable-review-bridge.js')],capture_output=True,text=True,encoding='utf-8',errors='replace');need(p.returncode==0,f'assessment-stable-review-bridge.js syntax error: {p.stderr}')

analytics_ui=text('assessment-analytics-ui.js')
for marker in ['Accuracy by difficulty','Most-selected wrong distractors','Exam pass rates','Export local analytics JSON','mouldmaster-question-analytics.json','MM_ASSESSMENT_ANALYTICS_REVIEW']: need(marker in analytics_ui,f'assessment analytics UI marker missing: {marker}')
...
p=subprocess.run(['node','--check',str(ROOT/'assessment-analytics-ui.js')],capture_output=True,text=True,encoding='utf-8',errors='replace');need(p.returncode==0,f'assessment-analytics-ui.js syntax error: {p.stderr}')

core=text('MouldMaster_Core_App.html');mark='window.MM_DATA = '
need(mark in core,'MM_DATA marker missing');D,_=json.JSONDecoder().raw_decode(core[core.index(mark)+len(mark):])
...
for(const level of ['Beginner','Intermediate','Advanced']){exams[level]={};for(const region of ['UK','US','NZ','ALL']){const arr=sandbox.window.getExamQuestions(level,region);exams[level][region]=arr.map(q=>({id:q.stableId,mmId:q.mmId,difficulty:q.difficulty,competency:q.competency,competencies:q.competencies||[q.competency],concept:q.concept,critical:q.critical,region:q.region||null,options:q.options,correct:q.correct,stem:q.q}))}}
const definitions=[];
...
for level,regions in runtime['exams'].items():
    for region,items in regions.items():
        expected=16 if region=='ALL' else 10;need(len(items)==expected,f'{level}/{region} exam count must be {expected}')
        ids=[x['id'] for x in items];need(all(ids),'every live item needs a stable ID');need(len(ids)==len(set(ids)),f'{level}/{region} has duplicate stable IDs');need(all(x['mmId']==x['id'] for x in items),f'{level}/{region} spaced-review ID must equal stable ID');need(all(len(x['options'])==4 and 0<=x['correct']<4 for x in items),f'{level}/{region} option/key integrity')
...
def norm_len(value): return len(re.sub(r'\s+',' ',str(value or '').strip()))
def longest_flag(item_id,options,correct,kind):
    lengths=[norm_len(x) for x in options];keyed=lengths[correct];longest_distractor=max(lengths[:correct]+lengths[correct+1:])
    if keyed>=longest_distractor:return {'id':item_id,'kind':kind,'correct_index':correct,'correct_length':keyed,'longest_distractor_length':longest_distractor,'lengths':lengths,'options':options}
    return None
strict_length_flags=[]
for q in definitions:
    need(len(q['options'])==4 and 0<=q['correct']<4,f"{q['id']} canonical option/key integrity")
    flag=longest_flag(q['id'],q['options'],q['correct'],q['kind']);
...
near=runtime['quality'].get('nearDuplicates',[]);runtime_leaks=runtime['quality'].get('answerLeakRisks',[]);cue_flags=[]
qualifiers=re.compile(r'\b(under|unless|provided|before|rather|while|current|validated|applicable|appropriate|within|depending|evidence)\b',re.I);absolutes=re.compile(r'\b(always|never|only|automatically|proves?|guarantees?|must)\b',re.I)
def technical_density(s): return sum(1 for w in re.findall(r"[A-Za-z][A-Za-z-]+",s) if len(w)>=10)
for q in live_technical.values():
    opts=q['options'];c=q['correct'];correct=opts[c];others=[o for i,o in enumerate(opts) if i!=c];clen=len(correct);med=sorted(len(o) for o in others)[1]
