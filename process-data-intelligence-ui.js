...
async function openAnalysis(datasetId){
  ...
  host.innerHTML=`<div data-pi-root><div class="di-actions" style="margin-bottom:12px"><button class="ghost" data-pi-library>← Dataset library</button><button class="ghost" data-pi-intake>Process-data intake</button></div><div class="card di-hero"><div class="eyebrow">Site-local process intelligence</div><h2>${esc(dataset.datasetMeta?.source_label||dataset.id)}</h2><p>${dataset.rowCount} prepared rows · ${dataset.quality?.analysisReady?'analysis-ready':'analysis-blocked'} · ${esc([dataset.entities?.machine,dataset.entities?.mould,dataset.entities?.materialGrade].filter(Boolean).join(' · '))}</p><div class="di-note"><b>Boundary:</b> these are site-local statistical evidence views. They do not create universal settings, machine safety limits, validated process windows, automatic root cause or production-change authority.</div></div>
...
function wire(host,dataset,rows){
  ...
  host.querySelector('[data-pi-drift]')?.addEventListener('click',async()=>{const id=host.querySelector('[data-pi-baseline]')?.value;if(!id)return;const result=await api.intelligence.compareToBaseline(dataset.id,id);host.querySelector('[data-pi-drift-result]').innerHTML=driftHtml(result)});
