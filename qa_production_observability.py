...
RUNTIME_TOKEN = "20260902.1-maturity-hardening-v2"
CACHE_REVISION = "maturity-hardening-v2-20260902"
...
require(f'RUNTIME_ASSET_VERSION="{RUNTIME_TOKEN}"' in index, "Browser runtime token does not match the active hardened bundle")
...
require(f"CACHE_REVISION='{CACHE_REVISION}'" in worker, "Service-worker hardened cache revision mismatch")
require(f'EXPECTED_STATIC_CACHE="mouldmaster-static-2026.08.26.2-{CACHE_REVISION}"' in index,
        "Bootstrap expected cache does not match hardened service-worker cache")
...
print(f"Production observability QA passed: local-only diagnostics {VERSION}, safe learner issue loop, coherent hardened browser/PWA cache identity, deployment/update/error probes, desktop/PWA coverage.")
