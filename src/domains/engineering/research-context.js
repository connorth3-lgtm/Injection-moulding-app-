/* MouldMaster engineering case -> governed research context adapter — 2026.09.04.1 */
...
function evidencePlan(record={},mechanismId){
  ...
  if(!candidate)return Object.freeze({schemaVersion:1,caseId:clean(record?.id)||null,mechanismId:mechanism,status:'not-supported-by-case-context',plan:null,boundary:BOUNDARY});
  const context=contextFromCase(record),plan=research.localEvidencePlan(context.input,mechanism);
  return Object.freeze({schemaVersion:1,caseId:clean(record?.id)||null,mechanismId:mechanism,status:'candidate-plan',plan:plan?Object.freeze({...plan,collect:Object.freeze((plan.collect||[]).slice()),sources:Object.freeze((plan.sources||[]).map(x=>Object.freeze({...x})))}):null,boundary:BOUNDARY});
}
...
return Object.freeze({version:VERSION,rankingFields:RANKING_FIELDS,excludedReasoningFields:EXCLUDED_REASONING_FIELDS,contextFromCase,analyzeCase,evidencePlan,analyzeCaseById,evidencePlanById,boundary:BOUNDARY,biasBoundary:BIAS_BOUNDARY});
});
