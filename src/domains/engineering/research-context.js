/* MouldMaster engineering case -> governed research context adapter — 2026.09.04.3 */
...
const PLAN_BOUNDARY='An evidence plan is available only for the top governed research match with at least two governed matched terms. Lower-ranked or one-term matches remain comparison context only and cannot produce a plan.';
...
function freezeApplicability(value){
  const x=value&&typeof value==='object'?value:{};
  return Object.freeze({score:Number.isFinite(Number(x.score))?Number(x.score):null,label:clean(x.label)||'unknown',matchedTerms:Object.freeze(Array.isArray(x.matchedTerms)?x.matchedTerms.map(clean).filter(Boolean):[]),reason:clean(x.reason)});
}
function candidateView(row,index){return Object.freeze({
  rank:index+1,
  mechanismId:row.id,
  title:row.title,
  evidenceState:row.evidenceState,
  evidenceQuality:row.evidenceQuality,
  applicability:freezeApplicability(row.applicability),
...
function analyzeCase(record={},limit=5){
  ...
  if(!context.hasContext)return Object.freeze({schemaVersion:1,caseId:clean(record?.id)||null,status:'no-context',sourceFields:context.sourceFields,excludedFields:context.excludedFields,candidates:Object.freeze([]),boundary:BOUNDARY,biasBoundary:BIAS_BOUNDARY,planBoundary:PLAN_BOUNDARY});
...
function evidencePlan(record={},mechanismId){
  ...
  const analysis=analyzeCase(record,12),candidate=analysis.candidates.find(x=>x.mechanismId===mechanism),top=analysis.candidates[0]||null;
  const matchCount=candidate?.applicability?.matchedTerms?.length||0;
  const eligible=!!candidate&&candidate.rank===1&&top?.mechanismId===mechanism&&matchCount>=MIN_PLAN_MATCH_TERMS;
  if(!eligible)return Object.freeze({schemaVersion:1,caseId:clean(record?.id)||null,mechanismId:mechanism,status:'not-supported-by-case-context',rank:candidate?.rank||null,applicability:candidate?.applicability||freezeApplicability(null),matchedTermCount:matchCount,plan:null,boundary:BOUNDARY,planBoundary:PLAN_BOUNDARY});
  const context=contextFromCase(record),plan=research.localEvidencePlan(context.input,mechanism);
  return Object.freeze({schemaVersion:1,caseId:clean(record?.id)||null,mechanismId:mechanism,status:'candidate-plan',rank:1,applicability:candidate.applicability,matchedTermCount:matchCount,plan:plan?Object.freeze({...plan,applicability:freezeApplicability(plan.applicability),collect:Object.freeze((plan.collect||[]).slice()),sources:Object.freeze((plan.sources||[]).map(x=>Object.freeze({...x})))}):null,boundary:BOUNDARY,planBoundary:PLAN_BOUNDARY});
}
...
return Object.freeze({version:VERSION,rankingFields:RANKING_FIELDS,excludedReasoningFields:EXCLUDED_REASONING_FIELDS,minPlanMatchTerms:MIN_PLAN_MATCH_TERMS,contextFromCase,analyzeCase,evidencePlan,analyzeCaseById,evidencePlanById,boundary:BOUNDARY,biasBoundary:BIAS_BOUNDARY,planBoundary:PLAN_BOUNDARY});
});
