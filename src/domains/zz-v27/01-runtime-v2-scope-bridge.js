...
const policy=`${R.policy||''} Learner storage is bound to MM_LEARNER_SCOPE opaque references in release ${VERSION}.`.trim();
Object.assign(R,{storage,snapshot(){const x=originalSnapshot()||{};return {...x,learnerToken:S.token(),scopeProvider:'MM_LEARNER_SCOPE',scopeBridgeVersion:VERSION}},scopeBridgeVersion:VERSION,policy});
window.MM_RUNTIME_V2=R;try{R.registerModule?.('runtime-v2-scope-bridge-v27',{version:VERSION,type:'learner-scope-bridge',status:'active'})}catch(_){}
})();
