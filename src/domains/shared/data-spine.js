/* MouldMaster canonical data spine — 2026.09.04.1 */
(function(){
'use strict';
...
function fingerprint(value){const text=typeof value==='string'?value:JSON.stringify(value,Object.keys(value||{}).sort());let h=2166136261;for(const ch of String(text)){h^=ch.charCodeAt(0);h=Math.imul(h,16777619)}return `fnv1a-${(h>>>0).toString(16).padStart(8,'0')}`}
...
function ingestSynchronous(){
  ...
  try{for(const ds of window.MM_PROCESS_EVIDENCE_DATASETS?.datasets||[]){register('process-case',ds.id,{title:ds.title||'',domain:ds.kind||''});for(const sid of ds.sourceIds||[])relation('process-case',ds.id,'evidence-source',sid,'supported-by',{granularity:'case'});for(const signal of Object.keys(ds.signals||{}))relation('process-case',ds.id,'process-signal',signal,'observes')}}catch(_){}
}
