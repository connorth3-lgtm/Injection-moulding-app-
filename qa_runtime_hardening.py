...
source_library = Path("source-library.js").read_text(encoding="utf-8")
lesson_depth = Path("lesson-evidence-depth.js").read_text(encoding="utf-8")
...
require('RUNTIME_ASSET_VERSION="20260826.2-lesson-evidence-depth"' in index, "bootstrap must expose the lesson-evidence-depth coherent runtime asset version")
...
require("EXPECTED_STATIC_CACHE=\"mouldmaster-static-2026.08.24.1-lesson-evidence-depth-20260826\"" in index, "browser bootstrap must know the current coherent PWA cache")
...
require("CACHE_REVISION='lesson-evidence-depth-20260826'" in service_worker and "${CACHE_VERSION}-${CACHE_REVISION}" in service_worker, "PWA cache revision must stay aligned with lesson evidence depth")
...
require("'./lesson-evidence-depth.js'" in service_worker, "targeted lesson evidence depth must be available offline")
require(index.index("assessment-evidence-sources.js") < index.index("lesson-evidence-depth.js") < index.index("assessment-evidence-approval.js"), "lesson evidence depth load order must follow evidence sources and precede approval/UI finalization")
...
courses=[
    'Foundations','Machine & Controls','Materials','Mould Design','Process Setup','Defect Troubleshooting',
    'Scientific Moulding','Capability & Validation','DOE & Statistics','Automation & Sensors',
    'Advanced Tooling & Simulation','Expert Process Engineering'
]
require("2026.08.25.2" in source_library and "COURSE_SOURCE_IDS" in source_library, "expanded lesson evidence library/version missing")
for course in courses:
    require(f"'{course}':" in source_library, f"lesson evidence fallback missing for course: {course}")
require("window.MM_EVIDENCE_SOURCES?.inferred?.(text)" in source_library, "lesson evidence must prefer topic-specific audited evidence mappings")
require("No general external source was auto-selected" in source_library and "fallback.remove()" in source_library, "empty lesson reference placeholder must be replaced when evidence is available")
require("slice(0,Math.max(0,5-existingCount))" in source_library, "lesson evidence panel must stay compact on mobile")
require("not universal production settings" in source_library, "lesson evidence panel must preserve the educational production-setting boundary")
require("data-mm-lesson-evidence-expanded" in source_library and "mm-lesson-evidence-links" in source_library, "lesson references must enrich the existing evidence card in place")

require("MM_LESSON_EVIDENCE_AUDIT" in lesson_depth and "topicSources" in lesson_depth and "fallbackSources" in lesson_depth, "targeted lesson evidence audit API missing")
require("EUROMAP 77 — IMM/MES data exchange" in lesson_depth and "FDA — Process Validation: General Principles and Practices" in lesson_depth and "Autodesk Moldflow — Draft Angle result" in lesson_depth, "targeted lesson source families missing")
require("not universal production recipes" in lesson_depth, "targeted lesson evidence must retain the no-universal-recipe education boundary")
