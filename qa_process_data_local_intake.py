...
const csv='timestamp,machine,mould,material_grade,material_lot,operator_id,customer_name,fill_time_s,cycle_time_s,cushion_mm,peak_cavity_pressure_mpa,mould_temp_c,machine_pressure_mpa,resin_moisture_ppm,quality_result,defect_code,comment\\n2026-08-26T10:00:00Z,IMM-A,Tool-X,PA66-GF30,LOT-SECRET,Alice-17,Customer One,1.20,28.4,4.5,83.2,45,72,320,PASS,WELD-LINE-SECRET,first shot\\n2026-08-26T10:00:30Z,IMM-A,Tool-X,PA66-GF30,LOT-SECRET,Bob-22,Customer One,1.24,28.7,4.4,84.1,46,74,330,FAIL,BURN-SECRET,second shot\\n';
...
for kept in ['shot_index','machine','mould','material_grade','material_lot','fill_time_s','cycle_time_s','cushion_mm','peak_cavity_pressure_mpa','mould_temp_c','machine_pressure_mpa','resin_moisture_ppm','quality_result','defect_code']:
...
need(row0['peak_cavity_pressure_mpa']==83.2 and row0['mould_temp_c']==45 and row0['machine_pressure_mpa']==72 and row0['resin_moisture_ppm']==320,'identifier words inside numeric signal names must not cause cavity/mould/machine/material-state measurements to be aliased')
...
need(numeric_ids['rows'][0]['peak_cavity_pressure_mpa']==83.4 and numeric_ids['rows'][0]['mould_temp_c']==45 and numeric_ids['rows'][0]['machine_pressure_mpa']==72 and numeric_ids['rows'][0]['resin_moisture_ppm']==320,'numeric moulding signals sharing identifier words must remain numeric evidence')
rule_actions={x['key']:x['action'] for x in numeric_ids['rules']}
need(rule_actions['machine_id']=='alias' and rule_actions['cavity_id']=='alias' and rule_actions['material_grade']=='alias','explicit identifier fields must classify as aliases before numeric inference')
...
for kept in ['shot_index','phase','dimension_value','dimension_unit','intervention_code','quality_result','defect_alias','resin_moisture_ppm']:
...
need(pilot['rows'][0]['resin_moisture_ppm']==320 and pilot['rows'][1]['resin_moisture_ppm']==520,'material-state numeric evidence must survive pilot preparation')
