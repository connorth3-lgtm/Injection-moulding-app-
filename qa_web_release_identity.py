...
preview=text('.pages-dist/preview/index.html')
preview_worker=text('.pages-dist/preview/service-worker.js')
need('http-equiv="refresh" content="0;url=../"' in preview,'legacy /preview/ URL must redirect to the production root')
need('<link rel="canonical" href="../">' in preview,'preview redirect must identify the production root as canonical')
need("Response.redirect(new URL('../',self.registration.scope),302)" in preview_worker,'legacy preview worker must redirect previously installed preview clients')
need('preview/index.html' in manifest.get('assets',{}),'preview compatibility redirect missing from Pages manifest')
need('preview/service-worker.js' in manifest.get('assets',{}),'preview compatibility worker missing from Pages manifest')
