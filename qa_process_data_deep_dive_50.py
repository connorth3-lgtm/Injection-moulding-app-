...
raw=[c for p in packs for c in p['cases']]
need(len(raw)==50,f'expected 50 additional cases, got {len(raw)}')
ids=[c[0] for c in raw]
need(len(set(ids))==50,'50-case IDs must be unique')
for c in raw:
    need(len(c)==8,f'{c[0]} compact case schema must have 8 fields')
    cid,title,kind,source_ids,signals,fault,diagnosis,next_evidence=c
...
for d in runtime['datasets']:
    need(len(d['rows'])==72,f"{d['id']} must generate exactly 72 cycles")
    phase=[r['phase'] for r in d['rows']]
    need(phase.count('baseline')==24 and phase.count('fault')==24 and phase.count('recovery')==24,f"{d['id']} must preserve 24/24/24 baseline-fault-recovery")
    need(d.get('phaseCounts')=={'baseline':24,'fault':24,'recovery':24},f"{d['id']} phaseCounts drifted")
    need(len(d['signals'])==4,f"{d['id']} generated signal count drifted")
...
for source_id,needle in {
 ...
 'ultrasound-review-2021':'34372430',
}.items():
    need(source_id in runtime['sources'],f'new deep-dive source not registered: {source_id}')
    need(needle in runtime['sources'][source_id]['url'],f'new deep-dive source URL drifted: {source_id}')
...
for f in ALL:
    ...
    need('../../'+f in {x.get('from') for x in pkg['build']['extraResources'] if isinstance(x,dict)},f'desktop package missing {f}')
...
qa=text('.github/workflows/qa.yml');desktop=text('.github/workflows/open-desktop-build.yml')
