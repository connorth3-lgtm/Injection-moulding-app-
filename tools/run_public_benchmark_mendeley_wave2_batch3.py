...
def profile_source(src):
    did, version = src["mendeleyDatasetId"], int(src["version"])
    ...
    return {
        "datasetId": src["datasetId"], "doi": src["doi"], "license": src["license"], "priority": src["priority"],
        "state": state, "apiFiles": api_items, "htmlFileIds": page_ids,
        "htmlOnlyFileIds": [x for x in page_ids if x not in known_ids],
        "fileCountLowerBound": len(set(page_ids) | known_ids), "errors": errors,
        "rawPayloadsDownloaded": False, "countsAsFullyProfiledMeasuredDataset": False, "acceptedMeasuredTimeSeriesSamples": 0,
    }
...
def main():
    ...
    contract = json.loads(CONTRACT.read_text(encoding="utf-8")); sources = [profile_source(x) for x in contract["sources"]]
    result = {
        "schema": 1, "status": "publisher-metadata-batch-profiled", "retrievedDate": args.retrieved_date, "sources": sources,
        "summary": {
            ...
            "fullyProfiledAccepted": 0, "acceptedMeasuredTimeSeriesSamples": 0, "rawPayloadsDownloaded": False,
        },
        "evidenceBoundary": "Stage 1 enumerates publisher metadata and file identifiers only. No third-party payload is downloaded and no reported measurement count is accepted until delivered files are source-profiled."
    }
