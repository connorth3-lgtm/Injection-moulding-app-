...
def validate(data: dict, require_resolved: bool = False) -> dict:
    ...
    if resolved:
        ...
        need(a1_files == mapped, "Analog Input[1] segment coverage must exactly match cycleFilesMapped")
        need(a2_files == mapped, "Analog Input[2] segment coverage must exactly match cycleFilesMapped")
