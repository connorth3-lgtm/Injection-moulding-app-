...
def compact_file(f, listing_source):
    ...
    low = name.lower()
    measured_hint = any(k in low for k in ["raw", "data", "experiment", "mechanical", "tensile", "hardness", "toughness"])
    derived_hint = any(k in low for k in ["prediction", "model", "code", "random", "decision", "class", "figure", "image", "sem"])
    ...
    return {
        ...
        "likelyExperimentalDataByName": bool(measured_hint and not derived_hint),
        "likelyDerivedOrSupportingByName": bool(derived_hint),
...
def compact_folder(f, listing_source):
    ...
    return {
        ...
        "listingSource": listing_source,
        "likelyExperimentalDataByName": any(k in low for k in ["raw", "data", "experiment", "mechanical", "tensile", "hardness", "toughness"]),
        "likelyDerivedOrSupportingByName": any(k in low for k in ["prediction", "model", "code", "random", "decision", "class", "figure", "image", "sem"])
    }
...
def page_file_links():
    ...
    for m in pattern.finditer(text):
        ...
        if fid in seen:
            continue
        seen.add(fid)
        context = text[max(0, m.start()-800):min(len(text), m.end()+800)]
...
def enumerate_manifest():
    attempts, files, folders = [], [], []
    for url in [f"{PUBLIC_API}/files?folder_id=root&version={VERSION}", f"{PUBLIC_API}/files?version={VERSION}"]:
        payload, diag = request_json_optional(url); attempts.append(diag)
        for item in flatten_list(payload):
            if str(item.get("type") or "").lower() == "folder": folders.append(compact_folder(item, url))
            else: files.append(compact_file(item, url))
    for url in [f"{PUBLIC_API}/folders?version={VERSION}", f"{PUBLIC_API}/folders?parent_id=root&version={VERSION}"]:
        payload, diag = request_json_optional(url); attempts.append(diag)
        for item in flatten_list(payload): folders.append(compact_folder(item, url))
    folder_by_id = {x["id"]: x for x in folders if x.get("id")}
    folders = list(folder_by_id.values())
    for folder in folders:
        url = f"{PUBLIC_API}/files?folder_id={urllib.parse.quote(str(folder['id']))}&version={VERSION}"
        payload, diag = request_json_optional(url); attempts.append(diag)
        for item in flatten_list(payload): files.append(compact_file(item, url))
    page_links, page_diag = page_file_links(); attempts.append(page_diag); files.extend(page_links)
    by_id, anonymous = {}, []
    for f in files:
        if f.get("id"):
            old = by_id.get(f["id"])
            if old is None or (not old.get("filename") and f.get("filename")): by_id[f["id"]] = f
        elif f.get("filename"): anonymous.append(f)
    return list(by_id.values()) + anonymous, folders, attempts
...
def main():
    ...
    result = {
        ...
        "source": {"datasetId":"mendeley-4h98rz9f92-v3","mendeleyDatasetId":DATASET_ID,"datasetDoi":DOI,"version":VERSION,"publisher":"Mendeley Data","datasetPage":PAGE,"licenseExpectedFromPublisherPage":"CC BY 4.0"},
        "manifest": {
            "files": files,
            "folders": folders,
            "fileCount": len(files),
            ...
            "likelyExperimentalFilesByName": [x["filename"] for x in files if x["likelyExperimentalDataByName"]],
            "likelyDerivedOrSupportingFilesByName": [x["filename"] for x in files if x["likelyDerivedOrSupportingByName"]],
            "listingAttempts": attempts,
            "rawPayloadsDownloaded": False,
            "rawRowsOrArraysEmitted": False
        },
        "acceptance": {"stage1ManifestComplete": bool(files or folders),"countsAsFullyProfiledMeasuredDataset":False,"acceptedMeasuredTimeSeriesSamples":0,"stage2Required":True},
        "evidenceBoundary": "Stage 1 enumerates publisher file/folder metadata and public-file link identifiers only. No file payload is downloaded. Paper-reported experimental rows do not count until delivered files are fingerprinted. Direct tensile modulus, toughness and hardness must be separated from classes, model outputs, figures, code and other derived/supporting material before acceptance."
    }
    ...
    print(json.dumps({"status":result["status"],"fileCount":result["manifest"]["fileCount"],"folderCount":result["manifest"]["folderCount"],"totalBytes":result["manifest"]["totalBytes"],"likelyExperimentalFilesByName":result["manifest"]["likelyExperimentalFilesByName"],"likelyDerivedOrSupportingFilesByName":result["manifest"]["likelyDerivedOrSupportingFilesByName"]}, indent=2))
