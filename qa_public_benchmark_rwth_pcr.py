...
need(s.get('downloadUrl') == 'https://publications.rwth-aachen.de/record/1016199/files/ExperimentalData.zip', 'RWTH current advertised archive URL drifted')
...
need(s.get('routeReviewed') == '2026-09-05', 'RWTH route recheck date drifted')
state = c.get('currentExecutionState') or {}
need(state.get('archiveRetrieved') is False, 'RWTH contract cannot claim retrieval before a valid ZIP execution')
need(state.get('acceptedMeasuredTimeSeriesSamples') == 0, 'RWTH contract cannot pre-count samples')
...
need(review.get('reviewed') == '2026-09-05', 'RWTH delivery-review date drifted')
...
need(archive.get('currentAdvertisedDownloadUrl') == s.get('downloadUrl'), 'RWTH contract must use the current publisher-advertised file path')
need(archive.get('historicalVersionedDownloadUrl', '').endswith('ExperimentalData.zip?version=1'), 'RWTH historical versioned route must remain documented')
prior = review.get('priorExecution') or {}
need(prior.get('workflowRun') == 33229910280, 'RWTH prior blocked execution provenance drifted')
need((review.get('publicSourceRecheck') or {}).get('promotionAllowedBeforeArchiveAndSemanticValidation') is False, 'RWTH route refresh must not weaken promotion gate')
...
print('MouldMaster RWTH PCR stage-1 benchmark QA passed (CC BY 4.0 source and current bare publisher archive route pinned; aggregate-only fail-closed retrieval; promotion remains disabled)')
