...
exports.isSafeExtractPath = isSafeExtractPath;
exports.moveDirAtomic = moveDirAtomic;
 ...
 */
function isSafeExtractPath(destPath, dir) {
 ...
 */
async function moveDirAtomic(src, dest) {
    var _a;
    // 4 retries → 5 total attempts; interval+backoff*attempt gives 250/500/750/1000 ms delays.
    let lastErr;
    try {
        await (0, builder_util_runtime_1.retry)(() => fs.rename(src, dest), {
            retries: 4,
            interval: 250,
            backoff: 250,
            shouldRetry: (err) => {
                var _a;
                if (TRANSIENT_RENAME_CODES.has((_a = err.code) !== null && _a !== void 0 ? _a : "")) {
                    builder_util_1.log.warn({ src: builder_util_1.log.filePath(src), dest: builder_util_1.log.filePath(dest), code: err.code }, "directory rename failed, retrying");
                    return true;
                }
                return false;
            },
    ...
    builder_util_1.log.warn({ src: builder_util_1.log.filePath(src), dest: builder_util_1.log.filePath(dest) }, "directory rename failed repeatedly; falling back to copy+delete");
    try {
...
async function extractArchive(file, dir) {
    ...
    try {
        ...
        if (file.endsWith(".tar.gz") || file.endsWith(".tgz")) {
            ...
            try {
                await (0, builder_util_1.exec)(cmd7za, ["x", "-bd", file, (0, builder_util_1.to7zaOutputSwitch)((0, builder_util_1.sanitizeDirPath)(decompressOutDir)), "-y"]);
            ...
            try {
                ...
                if (files.length === 0) {
                    builder_util_1.log.warn({ file: builder_util_1.log.filePath(file), tmpDir: builder_util_1.log.filePath(tmpDir), error: e.message }, "7z extraction produced no output");
