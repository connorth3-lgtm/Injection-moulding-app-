...
Target release family: open desktop `2026.08.26.8`  
...
- the published `MouldMaster-Academy-2026.8.26.8-x64.exe` from the tagged GitHub Release;
...
./desktop/electron/scripts/verify-real-windows-release.ps1 `
  -ExePath 'C:\path\MouldMaster-Academy-2026.8.26.8-x64.exe' `
...
2. Confirm the displayed desktop release is `2026.08.26.8`.
...
7. Confirm the import resets device-local assessment and Learning Insights analytics and reports the verified cleanup; the imported learner registry must not become active if analytics cleanup cannot be verified.
...
- [ ] imported analytics are reset and verified rather than attributed to new/imported profiles;
- [ ] a simulated/local cleanup failure blocks learner-registry replacement rather than claiming a clean import/reset;
