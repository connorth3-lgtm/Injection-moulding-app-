...
This is a smoke check, not a substitute for the broader manual accessibility protocol in `sources/ACCESSIBILITY_MANUAL_VALIDATION.md`.
...
After these pass, add the non-sensitive `data/legacy-windows-retirement-evidence.json` marker described in `sources/LEGACY_RETIREMENT_STATE.md`, remove the legacy executable/feed references in the same retirement PR, rerun full Release QA + Mobile Browser QA + Open Desktop Build, and publish the next desktop release.

## Evidence boundary

The public issue or repository marker may record the Windows version, MouldMaster release, pass/fail status, executable hash and non-sensitive defect/review information. Do **not** publish learner names, backup content, customer identifiers, proprietary process data or private filesystem/user information.
