...
const BASE_FILES=[
  ...
  'reading-patch.css','ui-shell.css','premium-ui.css','mobile-lesson-fix.css','learner-ux-repair.css','reading-patch.js','read-aloud.js','training-upgrade.js','training-qa-fix.js',
  ...
  'app-shell-registry.js','assessment-multimodal.js','pwa-shell.js','learning-experience.js','lesson-simple-experience.js','primary-learning-practice-hubs.js','learner-ux-repair.js','process-data-diagnostics.js','real-measured-data-assessment.js',
...
function filesUnder(rel){
  const root=path.join(ROOT,rel),out=[];
  if(!fs.existsSync(root))return out;
  const visit=(dir,prefix)=>{
    for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
      const child=path.join(dir,entry.name),name=`${prefix}/${entry.name}`;
      if(entry.isDirectory())visit(child,name);else if(entry.isFile())out.push(name)
    }
  };
  visit(root,rel);return out
}
