/* MouldMaster empirical learning-effectiveness layer — 2026.09.02.1 */
(function(){
'use strict';
...
document.addEventListener('click',e=>{if(e.target.closest?.('[data-mm-effectiveness-export]'))exportReport()});
let queued=false;function schedule(){if(queued)return;queued=true;(window.requestAnimationFrame||setTimeout)(()=>{queued=false;renderCard()},0)}new MutationObserver(schedule).observe(document.documentElement,{childList:true,subtree:true});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule);else schedule();
