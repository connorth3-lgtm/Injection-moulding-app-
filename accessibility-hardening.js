/* MouldMaster accessibility/browser hardening — 2026-09-01 */
(function(){
'use strict';
...
function decorateControls(root=document){
  for(const img of root.querySelectorAll?.('img:not([alt])')||[])img.alt='';
  for(const button of root.querySelectorAll?.('button')||[]){if(!clean(button.textContent)&&!button.getAttribute('aria-label')&&!button.getAttribute('aria-labelledby'))button.setAttribute('aria-label','Action')}
  for(const input of root.querySelectorAll?.('input,select,textarea')||[]){if(input.getAttribute('aria-label')||input.getAttribute('aria-labelledby')||input.id&&root.querySelector?.(`label[for="${CSS.escape(input.id)}"]`)||input.closest('label'))continue;const p=input.getAttribute('placeholder');if(p)input.setAttribute('aria-label',p)}
}
...
window.MM_ACCESSIBILITY_HARDENING=Object.freeze({version:VERSION,focusTrap:true,focusRestore:true,forcedColors:true,moreContrast:true,externalLinkIsolation:true,announce,scan,scope:'Runtime accessibility safeguards; formal WCAG conformance still requires manual assistive-technology and browser testing.'});
scan();
})();
