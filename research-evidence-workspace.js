/* MouldMaster research evidence workspace bridge — 2026.09.02.1 */
(function(){
'use strict';
const VERSION='2026.09.02.1';
function contextFromCase(c){c=c&&typeof c==='object'?c:{};return {text:[c.title,c.symptoms,c.evidence,c.hypotheses,c.tests,c.outcomes,c.notes].filter(Boolean).join(' '),materials:[c.materialFamily,c.material,c.materialGrade].filter(Boolean),process:[c.processFamily].filter(Boolean),tooling:[c.mould,c.mold,c.tool].filter(Boolean),signals:Array.isArray(c.signals)?c.signals:[],outcomes:Array.isArray(c.qualityOutcomes)?c.qualityOutcomes:[]}}
