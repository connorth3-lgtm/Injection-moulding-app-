...
for marker in ["VERSION='2026.09.06.1'","ANALYTICS_BASE='mm_assessment_analytics_v1'","TIMING_BASE='mm_assessment_exposure_timing_v1'","ROTATION_BASE='mm_assessment_opening_history_v1'","QUESTION_HISTORY_BASE='mm-assessment-question-history-v4'","RESULT_META_BASE='mm-assessment-result-meta-v1'","BASES=[ANALYTICS_BASE,TIMING_BASE,ROTATION_BASE,QUESTION_HISTORY_BASE,RESULT_META_BASE]","rotationKey:()=>scopedKey(ROTATION_BASE)","questionHistoryKey:()=>scopedKey(QUESTION_HISTORY_BASE)","resultMetaKey:()=>scopedKey(RESULT_META_BASE)","learnerScoped:true","MM_LEARNER_SCOPE","function hashScope","migrateFallbackScopes","partial-fail-closed","scopeProvider","migrateLegacy","clearAll","cancelInMemoryAttempt","wrapLearnerChange('switchUser')","wrapLearnerChange('createLearner')","after!==before){cancelInMemoryAttempt();clearAll()"]:
...
localStorage.setItem('mm-assessment-question-history-v4',JSON.stringify({'Beginner|ALL':[['legacy-q']]}));
localStorage.setItem('mm-assessment-result-meta-v1',JSON.stringify([{formFingerprint:'legacy-form'}]));
...
const aKey=api.analyticsKey(),aRotationKey=api.rotationKey(),aQuestionHistoryKey=api.questionHistoryKey(),aResultMetaKey=api.resultMetaKey();
...
if(JSON.parse(localStorage.getItem('mm-assessment-question-history-v4')||'{}')['Beginner|ALL'][0][0]!=='legacy-q')throw new Error('legacy recent-question history not migrated into learner scope');
if(JSON.parse(localStorage.getItem('mm-assessment-result-meta-v1')||'[]')[0].formFingerprint!=='legacy-form')throw new Error('legacy result metadata not migrated into learner scope');
...
localStorage.setItem('mm-assessment-question-history-v4',JSON.stringify({'Beginner|ALL':[['a1','a2']]}));
localStorage.setItem('mm-assessment-result-meta-v1',JSON.stringify([{formFingerprint:'form-a'}]));
if(!aRotationKey.startsWith('mm_assessment_opening_history_v1::')||!aQuestionHistoryKey.startsWith('mm-assessment-question-history-v4::')||!aResultMetaKey.startsWith('mm-assessment-result-meta-v1::'))throw new Error('learner A assessment stores are not scoped');
...
for(const key of ['mm_assessment_analytics_v1','mm_assessment_opening_history_v1','mm-assessment-question-history-v4','mm-assessment-result-meta-v1'])if(localStorage.getItem(key)!==null)throw new Error(`learner B can read learner A store: ${key}`);
...
localStorage.setItem('mm-assessment-question-history-v4',JSON.stringify({'Beginner|ALL':[['b1']]}));
localStorage.setItem('mm-assessment-result-meta-v1',JSON.stringify([{formFingerprint:'form-b'}]));
const bKey=api.analyticsKey(),bRotationKey=api.rotationKey(),bQuestionHistoryKey=api.questionHistoryKey(),bResultMetaKey=api.resultMetaKey();
if(aKey===bKey||aRotationKey===bRotationKey||aQuestionHistoryKey===bQuestionHistoryKey||aResultMetaKey===bResultMetaKey)throw new Error('compatibility learner scope keys collide');
...
if(localStorage.getItem('mm-assessment-question-history-v4')!==null||localStorage.getItem('mm-assessment-result-meta-v1')!==null)throw new Error('new learner inherited another learner assessment form history');
...
const strongAKey=api.analyticsKey(),strongARotationKey=api.rotationKey(),strongAQuestionHistoryKey=api.questionHistoryKey(),strongAResultMetaKey=api.resultMetaKey();
if(!strongAKey.endsWith('strong-learner-a')||!strongARotationKey.endsWith('strong-learner-a')||!strongAQuestionHistoryKey.endsWith('strong-learner-a')||!strongAResultMetaKey.endsWith('strong-learner-a'))throw new Error('shared learner scope token not used for learner A');
...
if(JSON.parse(localStorage.getItem('mm-assessment-question-history-v4')||'{}')['Beginner|ALL'][0][0]!=='a1')throw new Error('learner A recent-question history lost during shared-scope migration');
if(JSON.parse(localStorage.getItem('mm-assessment-result-meta-v1')||'[]')[0].formFingerprint!=='form-a')throw new Error('learner A result metadata lost during shared-scope migration');
...
if(JSON.parse(localStorage.getItem('mm-assessment-question-history-v4')||'{}')['Beginner|ALL'][0][0]!=='b1')throw new Error('learner B recent-question history lost during shared-scope migration');
if(JSON.parse(localStorage.getItem('mm-assessment-result-meta-v1')||'[]')[0].formFingerprint!=='form-b')throw new Error('learner B result metadata lost during shared-scope migration');
if(api.sharedMigration.migrated<8)throw new Error('expected assessment compatibility buckets were not migrated to shared scope');
...
for(const key of ['mm_assessment_analytics_v1','mm_assessment_opening_history_v1','mm-assessment-question-history-v4','mm-assessment-result-meta-v1'])if(localStorage.getItem(key)!==null)throw new Error(`confirmed reset did not clear learner assessment store: ${key}`);
...
process.stdout.write(JSON.stringify({version:api.version,learnerScoped:api.learnerScoped,aKey,bKey,strongAKey,strongAQuestionHistoryKey,strongAResultMetaKey,sharedMigration:api.sharedMigration}));
...
r=json.loads(p.stdout);need(r['version']=='2026.09.06.1' and r['learnerScoped'] is True,'assessment storage scope runtime metadata mismatch')
need(r['strongAKey'].endswith('strong-learner-a') and r['strongAQuestionHistoryKey'].endswith('strong-learner-a') and r['strongAResultMetaKey'].endswith('strong-learner-a'),'assessment storage runtime did not use shared learner-scope token')
...
need(V.get('assessment_storage_migration_version')=='2026.09.06.1','assessment storage migration hardening version missing')
...
print('MouldMaster learner-scoped assessment storage QA passed (including recent-question history and graded-form metadata, shared 128-bit learner scope, fail-closed migration, and verified reset/import cleanup)')
