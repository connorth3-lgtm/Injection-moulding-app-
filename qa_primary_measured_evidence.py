...
need(len(packs) == 4, f'expected 4 primary measured evidence packs, found {len(packs)}')
entries = []
pack_counts = {}
for p in packs:
    path = ROOT / p['path']
...
need(len(entries) == 31, f'expected 31 counted primary measured studies, found {len(entries)}')
need(summary.get('publisherVerifiedPeerReviewedPrimaryMeasured') == 31, 'summary primary measured total drifted')
need(summary.get('uniqueDois') == 31, 'summary unique DOI count drifted')
...
for i, e in enumerate(entries, 1):
    ...
    url = str(e.get('publisherUrl', '')).strip()
...
need(tier_counts == {'A': 3, 'B': 28}, f'credibility-tier counts drifted: {tier_counts}')
need(summary.get('tierA') == 3 and summary.get('tierB') == 28, 'manifest tier summary drifted')

by_doi = {e['doi'].lower(): e for e in entries}
candidates = index.get('promotionCandidates', [])
need(len(candidates) == 6, f'expected six staged promotion candidates, found {len(candidates)}')
need(summary.get('promotionCandidatesStagedNotApplied') == 6, 'promotion-candidate summary drifted')
seen_mechanisms = set()
for c in candidates:
    ...
    need(len(qn) == len(set(qn)), f'{mid}: same DOI used twice')
    ...
    need(('distinct' in rationale or 'separate' in rationale) and ('experiment' in rationale or 'programme' in rationale),
         f'{mid}: rationale must explain distinct experimental programmes')
...
print('MouldMaster primary measured evidence QA passed (31 unique peer-reviewed primary measured studies; 3 Tier A / 28 Tier B; 6 staged promotion candidates; no automatic status changes)')
