...
need(all(r.get('reviewer') and r.get('reviewedOn')=='2026-08-30' for r in records),'reviewer/date metadata incomplete')
...
need(len(runtime['diagnostic']['labs'])==9,'diagnostic lab runtime must expose 9 labs')
...
need("'./src/domains/runtime-packs/assessment-evidence-depth-runtime-pack.js'" in idx,"browser shell missing assessment runtime pack")
need(idx.index('evidence-runtime-pack.js')<idx.index('assessment-evidence-depth-runtime-pack.js'),'assessment approval runtime must load before the later evidence runtime pack')
