...
test('assessment focus UI is responsive and hides inactive controls',async({page})=>{
  ...
  for(const width of [320,360,390,412,1024]){
    ...
    for(let i=0;i<await steps.count();i++){
      ...
      expect(hit.width,`assessment step ${i+1} effective hit width at ${width}px`).toBeGreaterThanOrEqual(44);
      expect(hit.height,`assessment step ${i+1} effective hit height at ${width}px`).toBeGreaterThanOrEqual(44);
    }
