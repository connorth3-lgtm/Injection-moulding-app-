...
window.MM_LESSON_DEEP_AUTHORING_V2=Object.freeze({version:VERSION,total:records.length,records:records.map(x=>({...x,fingerprint:fingerprint(x)})),record:id=>byId[String(id)]||null,recordingMode:'runtime-v2 after-render hook',policy:'Every canonical lesson receives a lesson-specific mechanism/evidence/decision/teach-back record derived from its own authored summary, objectives, keypoints, exercise and safety context; duplicate generated records are rejected.'});
schedule();
})();
