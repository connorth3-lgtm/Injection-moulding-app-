...
for r in records:
    ...
    label=r.get('stem') or r.get('title') or r.get('labTitle') or r['id']
    if len(urls)<2: weak_urls.append({'id':r['id'],'label':label,'sourceIds':r.get('sourceIds',[]),'urls':urls})
    if len(auth)<2: weak_auth.append({'id':r['id'],'label':label,'authorities':sorted(auth),'sourceIds':r.get('sourceIds',[])})
need(not weak_urls,'formal questions below two distinct URLs: '+json.dumps(weak_urls,ensure_ascii=False))
need(not weak_auth,'formal questions below two authority families: '+json.dumps(weak_auth,ensure_ascii=False))
